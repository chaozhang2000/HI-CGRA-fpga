...
int main()
{
    ...
    sd_read_data("0:/bit1.bin",(u32)(bitstream_conv3),7744);
    FILINFO fileInfo2;
    f_stat("0:/bit1.bin",&fileInfo2);
    xil_printf("conv3.bin file size: %d \n\r",fileInfo2.fsize);

    sd_read_data("0:/fft0.bin",(u32)(bitstream_fft0),7744);
    FILINFO fileInfo1;
    f_stat("0:/fft0.bin",&fileInfo1);
    xil_printf("fft0.bin file size: %d \n\r",fileInfo1.fsize);

    sd_read_data("0:/fft1.bin",(u32)(bitstream_fft1),7744);
    FILINFO fileInfo3;
    f_stat("0:/fft1.bin",&fileInfo3);
    xil_printf("fft1.bin file size: %d \n\r",fileInfo3.fsize);

    sd_read_data("0:/fft2.bin",(u32)(bitstream_fft2),7744);
    FILINFO fileInfo4;
    f_stat("0:/fft2.bin",&fileInfo4);
    xil_printf("fft2.bin file size: %d \n\r",fileInfo4.fsize);
    ...
    XAxiDma_Config *config;
		...
		//fft results
		int fft_result_cgra[16] = {0};
		int fft_result_cpu[16] = {0};
    ...
    /*config cgra using fft.bin*/
        ...
        /*config cgra using fft1.bin*/
            print("start conifg cgra using fft1.bin\n\r");
            writecgrareg(0,1);
            print("change cgra state to 1\n\r");
            Xil_DCacheFlush();
            XAxiDma_SimpleTransfer(&axidma, (uintptr_t)bitstream_fft1, 7744, XAXIDMA_DMA_TO_DEVICE);
            print("dma transform fft1 config data to cgra \n\r");

            /*fft1 exe*/
            xil_printf("start exe fft1\n\r");
            writecgrareg(1<<2,0);
            if(readcgrareg(1<<2) == 1){
            	xil_printf("err finished reg is 1 before exe");
            	return;
            ...
            while(readcgrareg(0)!=0);
            XTime_GetTime(&t_fft_cgra_exeend);
            xil_printf("exe fft1 finish, finish reg = %d\n\r", readcgrareg(1<<2));

            /*config cgra using fft2.bin*/
                print("start conifg cgra using fft2.bin\n\r");
                writecgrareg(0,1);
                print("change cgra state to 1\n\r");
                Xil_DCacheFlush();
                XAxiDma_SimpleTransfer(&axidma, (uintptr_t)bitstream_fft2, 7744, XAXIDMA_DMA_TO_DEVICE);
                print("dma transform fft2 config data to cgra \n\r");

                /*fft2 exe*/
                xil_printf("start exe fft2\n\r");
                writecgrareg(1<<2,0);
                if(readcgrareg(1<<2) == 1){
                	xil_printf("err finished reg is 1 before exe");
                	return;
                ...
                while(readcgrareg(0)!=0);
                XTime_GetTime(&t_fft_cgra_exeend);
                xil_printf("exe fft2 finish, finish reg = %d\n\r", readcgrareg(1<<2));
        /*read fft result*/
        writecgrareg(2<<2,1);
        writecgrareg(3<<2,0);
        writecgrareg(4<<2,0);
        writecgrareg(5<<2,16);
        XAxiDma_SimpleTransfer(&axidma, (uintptr_t)fft_result_cgra, 16*4, XAXIDMA_DEVICE_TO_DMA);
        writecgrareg(0,4);
    ...
    print("start conifg cgra using conv3.bin\n\r");
    writecgrareg(0,1);
    print("change cgra state to 1\n\r");
    Xil_DCacheFlush();
    XAxiDma_SimpleTransfer(&axidma, (uintptr_t)bitstream_conv3, 7744, XAXIDMA_DMA_TO_DEVICE);
    ...
    xil_printf("load data %d\n\r",i);
      ...
      writecgrareg(3<<2,0);//startaddr
      writecgrareg(4<<2,0);//addaddr
      XAxiDma_SimpleTransfer(&axidma, (uintptr_t)datas[i], 200*4, XAXIDMA_DMA_TO_DEVICE);
      while(XAxiDma_Busy(&axidma, XAXIDMA_DMA_TO_DEVICE)){};
    }
    /*conv3 exe*/
    ...
    XAxiDma_SimpleTransfer(&axidma, (uintptr_t)conv3_result_cgra, 200*4, XAXIDMA_DEVICE_TO_DMA);
    writecgrareg(0,4);
    while(XAxiDma_Busy(&axidma, XAXIDMA_DEVICE_TO_DMA)){};
    for(int i = 0; i<200 ;i ++){
    ...
    xil_printf("exe time\n\r");
    xil_printf("exe time = %ld\n\r",t_conv3_cgra_exeend-t_conv3_cgra_exestart);
    ...
    xil_printf("cpu exe time\n\r");
    xil_printf("cpu exe time = %ld\n\r",t_conv3_cpu_exeend-t_conv3_cpu_exestart);
    for(int k = 0;k<200;k++){
